...
public class BM46 extends SI_Service {
	
	private GenDocInvoker genDocInvoker = null;
	private String docName = null;
	private String dependentRequired;
	
	public BM46(String docName, String data, boolean isMergeData, String siVersion, String processDataSource, String logDataSource, ServiceConfig serviceConfig, SI_Log log, String dependentRequired) {
	...
	public void businessMapping_Input() throws SIException {
		...
		if(!TT_YC_HDTSBD.isEmpty()) {
			JsonArray DS_BDS = TT_YC_HDTSBD.get(0).getAsJsonObject().has("DS_BDS")? 
							   ...
							   : new JsonArray();
			BO.add("TABLE_MOTATS", DS_BDS);
		} else {
			BO.add("TABLE_MOTATS", new JsonArray());
		}
	...
	public void businessMapping_Output() throws SIException {
		...
		/*DANH SACH TAI SAN BAO DAM*/
		if(!inputDataCFG.get("TABLE_MOTATS").getAsJsonArray().isEmpty()){
			JsonObject TABLE_VALUE_TEMP = new JsonObject();
			JsonArray TABLE_ARRAY_TEMP = new JsonArray();
			int STT = 1;
			
			for(JsonElement BDSEle : inputDataCFG.get("TABLE_MOTATS").getAsJsonArray()) {
				JsonObject QSSD = BDSEle.getAsJsonObject().has("QSSD") && !BDSEle.getAsJsonObject().get("QSSD").isJsonNull() ? 
								  ...
								  : new JsonObject();
				JsonObject QSH_SD_DDKD = BDSEle.getAsJsonObject().has("QSH_SD_DDKD") && !BDSEle.getAsJsonObject().get("QSH_SD_DDKD").isJsonNull() ? 
										BDSEle.getAsJsonObject().get("QSH_SD_DDKD").getAsJsonObject() 
										: new JsonObject();
				...
				TABLE_VALUE_TEMP.addProperty("TYPE_ASSETS", QSH_SD_DDKD.has("NGAY_GOP_VON") && !QSH_SD_DDKD.get("NGAY_GOP_VON").isJsonNull() ? QSH_SD_DDKD.get("NGAY_GOP_VON").getAsString():"");

				TABLE_VALUE_TEMP.addProperty("TABLE_DDKD", new JsonArray().toString());
			...
			inputDataCFG.addProperty("TABLE_MOTATS", TABLE_TSBD_VALUES.toString());
			
		} else {
			inputDataCFG.addProperty("TABLE_MOTATS", new JsonArray().toString());
		}
