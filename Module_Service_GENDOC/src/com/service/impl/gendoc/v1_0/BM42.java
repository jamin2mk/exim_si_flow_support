...
public class BM42 extends SI_Service {
	
	private GenDocInvoker genDocInvoker = null;
	private String docName = null;
	private String dependentRequired;
	
	public BM42(String docName, String data, boolean isMergeData, String siVersion, String processDataSource, String logDataSource, ServiceConfig serviceConfig, SI_Log log, String dependentRequired) {
	...
	public void businessMapping_Input() throws SIException {
		...
		JsonObject TT_BEN_BD = new JsonObject();
		...
		if(!TT_YC_HDTSBD.isEmpty()) {
			...
			TT_LIENLAC_CN = TT_BEN_BD.has("TT_LIENLAC") ? TT_BEN_BD.get("TT_LIENLAC").getAsJsonArray() : new JsonArray();
			TT_LIENLAC_NHP = TT_BEN_BD.has("TT_NGUOIHONPHOI") ? TT_BEN_BD.get("TT_NGUOIHONPHOI").getAsJsonObject().get("TT_LIENLAC").getAsJsonArray() : new JsonArray();
		}
	...
	public void businessMapping_Output() throws SIException {
		...
		if(!inputDataCFG.get("TABLE_TSBD").getAsJsonArray().isEmpty()){
			...
			for(JsonElement BDSEle : inputDataCFG.get("TABLE_TSBD").getAsJsonArray()) {
				JsonObject TS_CHCC = BDSEle.getAsJsonObject().has("TS_CHCC") && !BDSEle.getAsJsonObject().get("TS_CHCC").isJsonNull() ? 
								  BDSEle.getAsJsonObject().get("TS_CHCC").getAsJsonObject()
								  : new JsonObject();
				...
				TABLE_VALUE_TEMP.addProperty("HOME_NUMBER", TS_CHCC.has("CANHO_SO") && !TS_CHCC.get("CANHO_SO").isJsonNull() ? TS_CHCC.get("CANHO_SO").getAsString():"");
				TABLE_VALUE_TEMP.addProperty("FLOOR", TS_CHCC.has("TANG") && !TS_CHCC.get("TANG").isJsonNull() ? TS_CHCC.get("TANG").getAsString():"");
				TABLE_VALUE_TEMP.addProperty("FLOOR_AREA", TS_CHCC.has("DIEN_TICH") && !TS_CHCC.get("DIEN_TICH").isJsonNull() ? TS_CHCC.get("DIEN_TICH").getAsString():"");
				TABLE_VALUE_TEMP.addProperty("YEAR_BUILT", TS_CHCC.has("NAM_XD") && !TS_CHCC.get("NAM_XD").isJsonNull() ? TS_CHCC.get("NAM_XD").getAsString():"");
				TABLE_VALUE_TEMP.addProperty("CHCC_DCTN", TS_CHCC.has("DIA_CHI") && !TS_CHCC.get("DIA_CHI").isJsonNull() ? TS_CHCC.get("DIA_CHI").getAsString():"");
