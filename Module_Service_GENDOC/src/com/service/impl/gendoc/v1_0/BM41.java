...
public class BM41 extends SI_Service {
	
	private GenDocInvoker genDocInvoker = null;
	private String docName = null;
	private String dependentRequired;
	
	public BM41(String docName, String data, boolean isMergeData, String siVersion, String processDataSource, String logDataSource, ServiceConfig serviceConfig, SI_Log log, String dependentRequired) {
	...
	public void businessMapping_Output() throws SIException {
		...
		LGIssuesHandling.GetNumberFormater(inputData, "ACREAGE", "VN", 2);
		LGIssuesHandling.GetNumberFormater(inputData, "CONSTRUCTION_AREA", "VN", 2);
		...
		LGIssuesHandling.GetByNumberInWords(inputData, "ACREAGE", "BY_ACREAGE");
		LGIssuesHandling.GetByNumberInWords(inputData, "CONSTRUCTION_AREA", "BY_CONSTRUCTION_AREA");
		...
		if(!inputDataCFG.get("TABLE_TSBD").getAsJsonArray().isEmpty()){
			...
			for(JsonElement BDSEle : inputDataCFG.get("TABLE_TSBD").getAsJsonArray()) {
				...
				TABLE_VALUE_TEMP.addProperty("CONNEC_LAND_LOTTERY_NUMBER", TS_TRENDAT.has("TEN_NGUOI_SD") && !TS_TRENDAT.get("TEN_NGUOI_SD").isJsonNull() ? TS_TRENDAT.get("TEN_NGUOI_SD").getAsString():"");
				
				TABLE_VALUE_TEMP.addProperty("PROPERTY_DESCRIPTION_2", BDSEle.getAsJsonObject().has("MO_TA_THEM") && !BDSEle.getAsJsonObject().get("MO_TA_THEM").isJsonNull() ? BDSEle.getAsJsonObject().get("MO_TA_THEM").getAsString():"");
